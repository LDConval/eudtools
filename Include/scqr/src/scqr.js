...
// console.log(createQRData("bruh"));
//


// 128x128: each SL is 96x128 (normal size) in px

// 33size => 3072 (map pixel) in 128x128

// let k = 720;
// let l = 730;

// 128x128 map
// let qrUnits = createQRUnits("a very long text possibly fit with an url", {player0: 6, player1: 5, left: k, right: 4096 - k, top: l, bottom: 4096 - l})
//                 .sort((a,b) => a.x - b.x);

// 64x64 map
// let qrUnits = createQRUnits("a very long text possibly fit in a 64x64 sized map what", {player0: 6, player1: 5, mapSize: 2048, unitSize: 48, errorCorrectionLevel: "M"})
//                 .sort((a,b) => a.x - b.x);

// 96x96 map
// let qrUnits = createQRUnits("a very long text possibly fit with an url that is long long long", {player0: 6, player1: 5, mapSize: 3072, unitSize: 64, errorCorrectionLevel: "M"})
//                 .sort((a,b) => a.x - b.x);

// 192x192 map
// let qrUnits = createQRUnits("a very long text possibly fit with an url that is long long long long long", {player0: 6, player1: 5, mapSize: 6144, unitSize: 128, errorCorrectionLevel: "M"})
//                 .sort((a,b) => a.x - b.x);

// 256x256 map
// let qrUnits = createQRUnits("a very long text possibly fit with an url that is long long long long long long long long long long long long long", {player0: 6, player1: 5, mapSize: 8192, unitSize: 128, errorCorrectionLevel: "L"})
//                 .sort((a,b) => a.x - b.x);
// let qrUnits = createQRUnits("QQ群号1234567890", {player0: 6, player1: 5, mapSize: [8192, 8192], unitSize: 128, errorCorrectionLevel: "L"})
//                 .sort((a,b) => a.x - b.x);

// const fs = require("fs");
// let fileData = fs.readFileSync("chks/scenario256.chk");

// let unitsData = findChunk(fileData, "UNIT", validateUnitSection);
// let units = chkstructs.unpackAllUnits(unitsData.data);

// // let unitsNew = mergeQRUnits(units, qrUnits, [5, 6]);
// let unitsNew = mergeUnits(qrUnits, units);

// let unitsNewBuffer = chkstructs.packAllUnits(unitsNew);

// let newBuffer = replaceChunk(fileData, "UNIT", unitsNewBuffer, validateUnitSection);
...
// const fs = require("fs");
// let fileData = fs.readFileSync("chks/scenario_sat.chk");

// let qrTiles = getQRTiles("a very long text possibly fit with an url but that seemed not enough so i kinda made more i hope this is ok this time", {tile0: 0x40, tile1: 0x1a0, tileSize: 2});

// let dimData = findChunk(fileData, "DIM ");
// let dims = chkstructs.unpackDim(dimData.data);

// let tileData = findChunk(fileData, "MTXM");
// let tiles = chkstructs.unpackTile(tileData.data, [dims.width, dims.height]);
// tiles = setTile(tiles, qrTiles, 16, 16);

// let repackedTiles = chkstructs.packTile(tiles);
// let newBuffer = replaceChunk(fileData, "MTXM", repackedTiles);

// let tileDataSE = findChunk(fileData, "TILE");
// let tilesSE = chkstructs.unpackTile(tileDataSE.data, [dims.width, dims.height]);
// tilesSE = setTile(tilesSE, qrTiles, 16, 16);

// let repackedTilesSE = chkstructs.packTile(tilesSE);
// newBuffer = replaceChunk(newBuffer, "TILE", repackedTiles);
